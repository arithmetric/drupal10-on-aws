...
for arg in "$@"
do
    [ -z "$DRUSH_CMD_PARTS" ] || DRUSH_CMD_PARTS="$DRUSH_CMD_PARTS,"
    DRUSH_CMD_PARTS="$DRUSH_CMD_PARTS$arg"
done
DRUSH_CMD=$(echo $DRUSH_CMD_PARTS | sed -e 's/,/ /')

echo "Running Drush in AWS ECS"
echo ""
echo "  Command: drush $DRUSH_CMD"
echo "  Cluster: $CLUSTER_ARN"

aws ecs run-task \
  --cluster $CLUSTER_ARN \
  --count 1 \
  --group Drupal10-Drush \
  --launch-type FARGATE \
  ...
  --overrides "containerOverrides={name=web,command=[../vendor/bin/drush,$DRUSH_CMD_PARTS]}" \
